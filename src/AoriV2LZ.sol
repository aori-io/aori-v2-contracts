...
contract AoriV2 is IAoriV2, OApp {
    /*//////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    constructor(address _endpoint, address _serverSigner) OApp(_endpoint, _serverSigner) {
    ...
    function settleOrders(MatchingDetails calldata matching, bytes calldata serverSignature, bytes calldata hookData, bytes calldata options) external payable {
        ...
        // Require that this chainId by the taker is correct
        require(matching.takerOrder.inputChainId == block.chainid, "Taker order's input chain doesn't match current chain");
        ...
        // If the order is on the same chain, then we can just process the order directly
        if (matching.takerOrder.inputChainId == matching.takerOrder.outputChainId) {
        ...
        // If the order is on a different chain, then we need to peform a HTLC-like procedure to process
        // the orders. This is made easy using LayerZero. See _lzReceive for the rest of the logic.
        } else {
            // Lock tokens
            ...
            // Enable lock on the assets
            lockedBalances[matching.takerOrder.offerer][matching.takerOrder.inputToken] += matching.takerOrder.inputAmount;

            _lzSend(
                0, // Destination chain's endpoint ID.
                abi.encode(matching), // Encoded message payload being sent.
    ...
    function _lzReceive(
        ...
        // _message is the encoded version of a matching detail
        (MatchingDetails memory matching) = abi.decode(_message, (MatchingDetails));

        if (matching.makerOrder.inputChainId == block.chainid) {
            ...
            if (balances[matching.makerOrder.offerer][matching.makerOrder.inputToken] >= matching.makerOrder.inputAmount) {
                ...
                // Hook
                // IAoriHook(matching.makerOrder.offerer).beforeAoriTrade(); <- Understand how best to allow for hooks
                IERC20(matching.makerOrder.inputToken).safeTransferFrom(
                    matching.makerOrder.offerer,
                    address(this),
                    matching.makerOrder.inputAmount);
                // IAoriHook(matching.makerOrder.offerer).afterAoriTrade();
            }
            ...
            // then make a call back
            _lzSend(
                0, // Destination chain's endpoint ID.
                _message, // Encoded message payload being sent.
                options, // Message execution options (e.g., gas to use on destination).
                MessagingFee(msg.value, 0), // Fee struct containing native gas and ZRO token.
                payable(msg.sender) // The refund address in case the send call reverts.
            );
            ...
            // Check if maker wants to withdraw
            if (!matching.makerOrder.toWithdraw) {
                // Add balance
                balances[matching.makerOrder.offerer][matching.makerOrder.outputToken] += matching.makerOrder.outputAmount;
            } else {
    ...
    function claimUnfulfilled(MatchingDetails calldata matching, bytes calldata serverSignature) external {
        ...
        require(
            matching.blockDeadline < block.number,
            "Order execution deadline hasn't passed"
        );
        ...
        bytes32 takerOrderHash = getOrderHash(matching.takerOrder);
        require(!BitMaps.get(orderStatus, uint256(takerOrderHash)), "Taker order has already been settled");

        BitMaps.set(orderStatus, uint256(takerOrderHash));
        lockedBalances[matching.takerOrder.offerer][matching.takerOrder.inputToken] -= matching.takerOrder.inputAmount;
    }
